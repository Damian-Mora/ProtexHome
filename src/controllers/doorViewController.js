...
import { setActiveContext, clearActiveContext, getActiveSerial } from '../services/activeSerial.js';
...
export async function openDoorView(user, serial, onBack) {
  ...
  // Estado inicial (sin ping automático)
  updateDoorUI(device.status);

  // Historial: carga inicial + escucha en tiempo real
  const canDelete = userRole === 'PRO' || userRole === 'ADM';
  const historyController = new HistoryController(serial);
  const initialHistory = await historyController.loadHistory(20);
  historyController.renderHistory(initialHistory, 'history-list', canDelete);
  const historyUnsub = historyController.listenNewEvents((events) => {
  ...
  // Array para guardar unsubscribers
  const deviceUnsubscribers = [];

  // Escuchar estado online/offline
  const unsubOnline = controller.listenOnlineStatus((status) => {
    const activeSerial = getActiveSerial();
    if (activeSerial === serial) {
      updateDoorUI(status);
    } else {
      console.log(`⛔ Ignorada actualización de estado para ${serial} (serial activo ${activeSerial})`);
    }
  });
  deviceUnsubscribers.push(unsubOnline);

  // Escuchar posición de puertas
  const unsubPosition = controller.listenDoorPosition((doorNumber, state) => {
    const activeSerial = getActiveSerial();
    if (activeSerial !== serial) {
      console.log(`⛔ Ignorada posición de puerta ${doorNumber} de ${serial} (serial activo ${activeSerial})`);
      return;
    }
  ...
  deviceUnsubscribers.push(unsubPosition);

  // Guardar unsubscribe del historial
  deviceUnsubscribers.push(historyUnsub);
